...
PlaylistComponent::PlaylistComponent() {
    ...
    tableComponent.getHeader().addColumn("Number :", 1, 100);
    tableComponent.getHeader().addColumn("Title : ", 2, 100);
    tableComponent.getHeader().addColumn("Duration Of Track :", 3, 100);
    ...
    addTracks.setButtonText("Add Track from Library");
    addTracks.addListener(this);

    addAndMakeVisible(removeTracks);
    removeTracks.setButtonText("Remove Track from List");
...
void PlaylistComponent::paintCell(juce::Graphics& g, int rowNumber, int columnId, int width, int height,
    ...
    auto duration = floor(track.getDuration() / 60 * 200) / 200;

    if (columnId == 1) // Number
    {
